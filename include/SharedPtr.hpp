...
class SharedPtr {
 public:
  SharedPtr() : totalUse(new std::atomic<size_t>{1}) {}

  explicit SharedPtr(T *ptr)
      : pointer(ptr), totalUse(new std::atomic<size_t>{1}) {}

  SharedPtr(const SharedPtr &shared) : totalUse(nullptr) { *this = shared; }

  SharedPtr(SharedPtr &&shared) noexcept : totalUse(nullptr) {
  ...
  // возвращает количество объектов SharedPtr, которые ссылаются 
  // на тот же управляемый объект
  [[nodiscard]] size_t use_count() const { return *totalUse; }
